...
fn app() -> Element {
    ...
    // line is already an answer to "where to", and taking focus from it would
    // put the caret in front of a page the user asked for.
    //
    // This needs the engine's queued focus (`dioxus-native-dom`): `set_focus`
    // used to take `doc_mut()` synchronously, and `DioxusDocument::poll` drives
    // task wakeups while holding that borrow, so asking here panicked on every
    // launch with `RefCell already borrowed`.
